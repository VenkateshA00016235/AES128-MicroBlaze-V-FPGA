...
static void mix_columns(uint8_t state[16])
{
    ...
    for (column = 0; column < 4; column++) {
        ...
        uint8_t total = a0 ^ a1 ^ a2 ^ a3;
        uint8_t original_a0 = a0;

        state[i]     ^= total ^ xtime(a0 ^ a1);
        state[i + 1] ^= total ^ xtime(a1 ^ a2);
        state[i + 2] ^= total ^ xtime(a2 ^ a3);
        state[i + 3] ^= total ^ xtime(a3 ^ original_a0);
    }
}
